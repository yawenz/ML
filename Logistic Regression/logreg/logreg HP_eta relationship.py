...
class LogReg:
    ...
    def sg_update(self, train_example, iteration, use_tfidf=False):
        ...
        # term_2 for egularized update
        t2 = 1 - 2 * self.lam * self.eta(0)
        
        for i in nonzero(train_example.x)[0].tolist():
            # for x_i not equal 0 conduct unegularized update
            self.w[i] +=  t1 * train_example.x[i] * self.eta(0)
            if i != 0:
...
if __name__ == "__main__":
    ...
    thefile = open("/Users/yawen/desktop/CSCI 5622 Machine Learning/Homework 2/logreg/eta_HP.txt", "w")
    
    for eta in [0.001, 0.01, 0.1, 1, 10, 100]:
        args.eta = eta
        ...
        for pp in xrange(args.passes):
            ...
            for ex in train:
                ...
                if iteration % 10 == 0:
                    ...
                    thefile.write("eta %f\tHP %f\n" %(args.eta, ho_lp))
                    print("eta %f\tHP %f" %(args.eta, ho_lp))
                iteration += 1
